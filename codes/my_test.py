...
for test_loader in test_loaders:
    ...
    for data in test_loader:
        ...
        import os
        img_path = os.path.join(dataset_dir, data['folder'][0])
        idx, max_idx = data['idx'][0].split('/')
        img_name = '{:04d}'.format(int(idx))
        ...
        suffix = opt['suffix']
        if suffix:
            save_img_path = osp.join(img_path, str(img_name) + suffix + '.png')
        else:
            save_img_path = osp.join(img_path, str(img_name) + '.png')
        util.mkdir(img_path)
